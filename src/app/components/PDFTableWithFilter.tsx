...
export function PDFTableWithFilter({ items, title, viewMode, selectedCategorySubject }: PDFTableWithFilterProps) {
  ...
  const renderTable = (tests: EnhancedTestRecord[], tableTitle: string, id?: string) => {
    ...
    const getResponsiveMode = (): 'year-with-audio' | 'year-no-audio' | 'subject-no-audio' | 'subject-with-audio' | 'overview-with-audio' => {
      if (viewMode === 'byYear') {
        // 年度別
    ...
    return (
      <div id={id} className="mb-8 flex flex-col gap-3">
        ...
        <div className="bg-white border border-[var(--color-table-border)] rounded overflow-hidden">
              ...
              {tests.map((item, index) => (
                ...
                >
                  ...
                  <td className="p-1.5 sm:p-2 border-r border-[var(--color-table-border)] overflow-hidden">
                    <PDFActionGroup pdfPath={item.questionPdf} pdfState={item.pdfState} type="question" responsiveMode={responsiveMode} />
                  </td>
                  {/* 解答 */}
                  <td className={`p-1.5 sm:p-2 ${isListening ? 'border-r border-[var(--color-table-border)]' : ''} overflow-hidden`}>
                    <PDFActionGroup pdfPath={item.answerPdf} pdfState={item.pdfState} type="answer" responsiveMode={responsiveMode} />
                  </td>
                  {/* 音声（リスニングの場合のみ） */}
                  {isListening && (
                    <td className="p-1.5 sm:p-2 overflow-hidden">
                      <AudioActionGroup audioPath={item.audio} pdfState={item.pdfState} priority={item.priority} responsiveMode={responsiveMode} />
                    </td>
                  )}
                </tr>
              ))}
